/* usampler.c - admin-free user-mode sampling profiler (Very-Sleepy style).
 *
 * Pure C89 port of usampler.cpp (no STL / no C++), so it builds with older
 * toolchains. Launches a child process, periodically suspends each of its
 * threads, reads the instruction pointer (RIP), resumes, records the sample,
 ...
 * launched itself).
 *
 * Build (MSVC):  cl /nologo /O2 /TC usampler.c /link dbghelp.lib winmm.lib
 * Build (MinGW): gcc -O2 usampler.c -o usampler.exe -ldbghelp -lwinmm
 *
...
static ModInfo* g_mods   = NULL;  static size_t g_mn = 0, g_mcap = 0;
/* Cache of the target's thread handles (see refreshThreads). */
typedef struct { DWORD tid; HANDLE h; int seen; } TgtThread;
static TgtThread* g_thr = NULL;  static size_t g_tn = 0, g_tcap = 0;
static Hit*     g_hits   = NULL;  static size_t g_hcap = 0, g_hcount = 0;
    ...
    for (attempt = 0; attempt < 10; attempt++) {
        ...
        if (Module32First(snap, &me)) {
            do {
                ...
                for (k = 0; k < g_mn; k++) if (g_mods[k].base == base) { found = 1; break; }
                if (!found) {
                    if (g_mn == g_mcap) {
...
static void refreshThreads(void) {
    ...
    THREADENTRY32 te;
    size_t k, w;
    for (k = 0; k < g_tn; k++) g_thr[k].seen = 0;
    ...
    if (Thread32First(tsnap, &te)) {
        do {
            ...
            for (k = 0; k < g_tn; k++)
                if (g_thr[k].tid == te.th32ThreadID) { g_thr[k].seen = 1; have = 1; break; }
            if (!have) {
    ...
    w = 0;
    for (k = 0; k < g_tn; k++) {
        if (g_thr[k].seen) { if (w != k) g_thr[w] = g_thr[k]; w++; }
        else CloseHandle(g_thr[k].h);
    }
    g_tn = w;
}
...
static int modFor(DWORD64 a) {
    size_t k;
    for (k = 0; k < g_mn; k++)
        if (a >= g_mods[k].base && a < g_mods[k].base + g_mods[k].size) return (int)k;
    return -1;
}
...
static void waitInterval(void) {
    LARGE_INTEGER a, b;
    if (g_sleepUs >= 1000) { Sleep((DWORD)(g_sleepUs / 1000)); return; }
    QueryPerformanceCounter(&a);
    for (;;) {
        QueryPerformanceCounter(&b);
        if ((double)(b.QuadPart - a.QuadPart) * g_qpcToUs >= (double)g_sleepUs) break;
        YieldProcessor();
    }
}
...
static void dumpNC(NameCount* arr, size_t n, double denom, int skipIdle,
    ...
    for (i = 0; i < n && shown < topN; i++) {
        if (skipIdle && isIdle(arr[i].name)) continue;
...
int main(int argc, char** argv) {
    ...
    for (; ci < argc; ci++) {
        ...
        if (strcmp(a, "--") == 0) { ci++; break; }
        else if (strcmp(a, "--out") == 0 && ci + 1 < argc)      { outPath = argv[++ci]; }
        else if (strcmp(a, "--sleep-us") == 0 && ci + 1 < argc) { g_sleepUs = atol(argv[++ci]); }
        else if (strcmp(a, "--skip-ms") == 0 && ci + 1 < argc)  { skipMs = atol(argv[++ci]); }
    ...
    for (i = (size_t)ci; i < (size_t)argc; i++) {
        ...
        if (need > cmdcap) { cmdcap = need * 2; cmd = (char*)realloc(cmd, cmdcap); }
    ...
    while (WaitForSingleObject(pi.hProcess, 0) == WAIT_TIMEOUT) {
        ...
        if (!warmupDone && ms >= (double)skipMs) warmupDone = 1;

        for (k = 0; k < g_tn; k++) {
            CONTEXT ctx;
            if (SuspendThread(g_thr[k].h) == (DWORD)-1) continue;
            ZeroMemory(&ctx, sizeof(ctx));
            ctx.ContextFlags = CONTEXT_CONTROL;
            if (GetThreadContext(g_thr[k].h, &ctx)) {
                if (warmupDone) { hits_add(ctx.Rip); total++; }
                else            { skipped++; }
            }
    ...
    {
        size_t k;
        for (k = 0; k < g_tn; k++) CloseHandle(g_thr[k].h);
        g_tn = 0;
    }
    timeEndPeriod(1);
    ...
    for (i = 0; i < g_hcap; i++) {
        DWORD64 a; unsigned int c; int mi; const char* modName;
        char fn[512];
        SYMBOL_INFO* sym;
        DWORD64 disp = 0;
        if (!g_hits[i].count) continue;
        a = g_hits[i].addr; c = g_hits[i].count;
        mi = modFor(a);
        ...
        if (mi >= 0 && SymFromAddr(hCur, a, &disp, sym)) {
            ...
            if (flen < 0 || flen >= (int)sizeof(fn)) flen = (int)sizeof(fn) - 1;
    ...
    for (i = 0; i < nFunc; i++) if (isIdle(byFunc[i].name)) idleSamples += byFunc[i].count;
    ...
    free(cmd); free(g_hits); free(g_mods); free(byFunc); free(byMod);
    return 0;
}
