#pragma warning (disable:4267) // conversion from 'size_t' to 'const int'
...
std::map<std::string, std::vector<std::string>> get_dir_keyed_files(const std::string & path, uint32_t max_files_per_dir = 100) {
    ...
    do {
        ...
        if (fileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            // printf("[%s]: processing [%s] in [%s]\n", __func__, fullPath.c_str(), parentDir.c_str());
