...
bool layer_window_manager::convert_layers_to_aliased_cache() {
    ...
    for (auto & [layer_idx, locs] : layer_tensors) {
        ...
        if (!fill_ok) { VirtualFree(anon, 0, 0x8000 /*MEM_RELEASE*/); return false; }
        ...
        if (!buf) {
            ...
            VirtualFree(anon, 0, 0x8000 /*MEM_RELEASE*/);
            return false;
        }
        ...
        converted_layers++;
        converted_bytes += total;
    }

    if (converted_layers > 0) {
        aliased_cache = true;
        printf("layer_window: aliased-cache active — %zu layers converted to imported "
               "anonymous buffers (%.1f MiB), zero-copy compute (Stage 1: all resident)\n",
               converted_layers, converted_bytes / (1024.0 * 1024.0));
        fflush(stdout);
    }
