...
struct vk_buffer_struct {
    ...
    std::vector<uint32_t> resource_to_page;    // resource page ΓåÆ backing page (UINT32_MAX = unmapped)
    ...
    bool commit_range(size_t offset, size_t byte_size) {
        ...
        for (uint32_t p = start_page; p <= end_page; p++) {
            ...
            if (bp == UINT32_MAX) {
                GGML_ABORT("Vulkan sparse heap overflow: no free pages ΓÇö increase --weight-budget or reduce model size "
...
static vk_buffer ggml_vk_create_sparse_buffer(vk_device& device, size_t size, size_t budget) {
    ...
    // No bindBufferMemory for sparse buffers ΓÇö pages are bound via vkQueueBindSparse
    buf->device = device;
    buf->size = size;
