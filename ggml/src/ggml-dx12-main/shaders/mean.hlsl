// mean.hlsl - per-row mean: dst[0,i1,i2,i3] = mean_i0(src0[i0,i1,i2,i3])
// Output has ne0=1. One thread group per output row.
...
void main(uint3 gtid : SV_GroupThreadID, uint3 gid : SV_GroupID) {
    ...
    if (local_id == 0) {
        ...
        float mean = shared_sum[0] / (float)ne00;
        store_auto(dst, off_dst, mean, dst_esize);
    }
}
