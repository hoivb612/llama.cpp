// Register-blocked Q4_K x Q8_1 MUL_MAT using packed int8 dot products.
//
// Same structure as mul_mat_q8_0_q8_1_mmq.hlsl: MMQ_TM x MMQ_TN outputs per
// thread and quad-major groupshared tiles, which is what keeps the inner-loop
// reads off a single LDS bank. Q4_K adds the super-block minimum, so each
// 32-element block carries a scale and a min instead of just a scale.
//
...
uint q4k_byte(ByteAddressBuffer buf, uint byte_off) {
    ...
    for (uint block = 0; block < num_q8_blocks; block++) {
        ...
        [unroll(MMQ_WITER)] for (uint r = ld_row; r < MMQ_BN; r += MMQ_LDROWS) {
            ...
            if (gn < ne01) {
                ...
                const uint blk_off = row_off + q4_block * Q4K_BSIZE;
                const uint4 raw = src0.Load4(blk_off + 16u + il * 32u
                                             + ld_part * 4u);
                wv = high_nibble ? ((raw >> 4u) & 0x0F0F0F0Fu)
                                 : (raw & 0x0F0F0F0Fu);
                ...
                if (ld_part == 0u) {
                    ...
                    const uint sc = (q4k_byte(src0, sc_off + scidx0) & 0x0Fu)
                        | ((q4k_byte(src0, sc_off + scidx1) & scmask1) >> scshft1);
                    const uint m = ((q4k_byte(src0, sc_off + mbidx0) & mbmask0) >> mbshft0)
                        | ((q4k_byte(src0, sc_off + mbidx1) & mbmask1) >> mbshft1);
