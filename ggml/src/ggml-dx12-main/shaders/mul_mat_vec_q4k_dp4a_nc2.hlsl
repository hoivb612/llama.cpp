// mul_mat_vec_q4k_dp4a_nc2.hlsl - Q4_K dp4a matvec for small-M batch (M==2).
//
// Computes NUM_ROWS=2 output rows x NUM_COLS=2 output cols per workgroup,
// loading each Q4_K super-block once and reusing the decoded weights across
// both activation columns. Closes the M=2..8 gap where the existing wmma
// tiled batch path (fl=4) wastes 16x of its 32x32 tile.
//
// Dispatch: groups_x = (N+1)/2, groups_y = 1, groups_z = ne2*ne3
// Gated to ne11==2, ne12==1, ne13==1 (broadcast handled by caller).
//
// src1 is the Q8_1-quantized activation in the scratch buffer (laid out
// flat: ((i3*ne12 + i2)*ne11 + col)*num_q8_per_vec + block).
...
void main(uint3 group_id : SV_GroupID, uint tid : SV_GroupIndex) {
    ...
    for (uint block_idx = ix; block_idx < num_blocks; block_idx += it_size) {
        // Decode Q4_K row data ONCE per super-block, reused across both cols.
        ...
        // --- Col 0 ---
        {
            uint q8_super_base = q8_col0_base + block_idx * 8 * Q8_1_BSIZE;
            ...
            acc00 += compute_dp4a_row(dall0, dmin0, sc00, sc01, sc02, sc03, sc04, sc05, sc06, sc07,
                                      qs0_0, qs64_0, q8d0, q8d1, q8d2, q8d3,
                                      q8_qs0, q8_qs1, q8_qs2, q8_qs3,
                                      q8_psum0, q8_psum1, q8_psum2, q8_psum3);
            acc10 += compute_dp4a_row(dall1, dmin1, sc10, sc11, sc12, sc13, sc14, sc15, sc16, sc17,
        ...
        // --- Col 1 ---
        {
            uint q8_super_base = q8_col1_base + block_idx * 8 * Q8_1_BSIZE;
            uint q8_off0 = q8_super_base + q8_sub0 * Q8_1_BSIZE;
            uint q8_off1 = q8_super_base + q8_sub1 * Q8_1_BSIZE;
            uint q8_off2 = q8_super_base + q8_sub2 * Q8_1_BSIZE;
            uint q8_off3 = q8_super_base + q8_sub3 * Q8_1_BSIZE;
            ...
            acc01 += compute_dp4a_row(dall0, dmin0, sc00, sc01, sc02, sc03, sc04, sc05, sc06, sc07,
                                      qs0_0, qs64_0, q8d0, q8d1, q8d2, q8d3,
                                      q8_qs0, q8_qs1, q8_qs2, q8_qs3,
                                      q8_psum0, q8_psum1, q8_psum2, q8_psum3);
            acc11 += compute_dp4a_row(dall1, dmin1, sc10, sc11, sc12, sc13, sc14, sc15, sc16, sc17,
                                      qs0_1, qs64_1, q8d0, q8d1, q8d2, q8d3,
                                      q8_qs0, q8_qs1, q8_qs2, q8_qs3,
                                      q8_psum0, q8_psum1, q8_psum2, q8_psum3);
        }
    ...
    // Wave-portable reduction for all 4 accumulators.
    ...
    if (tid == 0) {
        ...
        // dst layout: dst[col, row] flat — col uses nb1, row uses nb0.
