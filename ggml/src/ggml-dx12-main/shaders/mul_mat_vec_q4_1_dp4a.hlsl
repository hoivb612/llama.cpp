// mul_mat_vec_q4_1_dp4a.hlsl - dp4a-accelerated Q4_1 matvec (M=1)
//
// Q4_1 block (20 bytes, 4-byte aligned):
//   offset 0..1   : d (f16)
//   offset 2..3   : m (f16)
//   offset 4..19  : qs[16] (qs[l] low nibble = elem l, high nibble = elem l+16)
//
// Element l = (qs[l%16] nibble) * d + m, nibble range [0, 15]
//
// The unsigned nibbles already fit in int8, so dp4a consumes them directly and
// the affine 'm' term folds in via the Q8_1 's' field (= d_a * sum(a_int8)):
//   block_sum = w_d * a_d * dp4a_sum + w_m * s
// 8 lanes share each block, so each lane adds w_m * s / 8.
//
// Structurally identical to mul_mat_vec_q4_0_dp4a.hlsl; Q4_0's -8 bias
// correction is replaced by Q4_1's additive minimum. 32 threads,
// 8 threads/block x 4 blocks/iter, 2 rows/group sharing activations.
...
uint read_u32_q4_1(ByteAddressBuffer buf, uint byte_off) {
...
void main(uint3 group_id : SV_GroupID, uint tid : SV_GroupIndex) {
    ...
    for (uint block_iter = 0; block_iter < num_blocks; block_iter += BLOCKS_PER_ITER) {
        ...
        if (block_idx < num_blocks) {
            ...
            // --- Row 0 ---
            {
                uint w_off = src0_row0 + block_idx * Q4_1_BSIZE;
                uint dm = read_u32_q4_1(src0, w_off);
                float w_d = f16_to_f32(dm & 0xFFFFu);
                float w_m = f16_to_f32(dm >> 16);
                uint qs4 = read_u32_q4_1(src0, w_off + 4 + l_in_half);
                uint w_packed = (half_sel == 0u) ? (qs4 & 0x0F0F0F0Fu) : ((qs4 >> 4) & 0x0F0F0F0Fu);
                int isum = 0;
                isum = dot4add_i8packed(w_packed, a_packed, isum);
                // w_m * a_s is per-block; 8 lanes per block -> 1/8 each.
                acc0 += w_d * a_d * float(isum) + w_m * a_s * 0.125f;
            }

            // --- Row 1 ---
            {
                uint w_off = src0_row1 + block_idx * Q4_1_BSIZE;
                uint dm = read_u32_q4_1(src0, w_off);
                float w_d = f16_to_f32(dm & 0xFFFFu);
                float w_m = f16_to_f32(dm >> 16);
                uint qs4 = read_u32_q4_1(src0, w_off + 4 + l_in_half);
                ...
                int isum = 0;
                isum = dot4add_i8packed(w_packed, a_packed, isum);
                acc1 += w_d * a_d * float(isum) + w_m * a_s * 0.125f;
            }
        }
    }
