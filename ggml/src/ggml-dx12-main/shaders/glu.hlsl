...
float glu_activation(float x, uint glu_op, float alpha, float limit) {
    switch (glu_op) {
        ...
        case 1: {  // GEGLU
            ...
            // tanh(x) = 1 - 2/(exp(2x)+1).  Avoids HLSL tanh() which has been
            // observed to return NaN on Intel B390 for large |arg|.
            float aa = x * x;
            float val = 0.7978845608f * x * (1.0f + 0.044715f * aa);
            float e = exp(min(2.0f * val, 88.0f));  // clamp to avoid Inf*0=NaN
...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    if (split_mode) {
        ...
        gate_val = load_auto(src0, off0, src0_esize);
        up_val   = load_auto(src1, off1, src1_esize);
    } else {
        uint nc = ne0;
        ...
        gate_val = load_auto(src0, off_gate, src0_esize);
        up_val   = load_auto(src0, off_up,   src0_esize);
    }
