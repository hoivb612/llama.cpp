// mul_mat_q4_0_wmma.hlsl - Register-blocked tiled MUL_MAT with Q4_0 weights
// Mirrors mul_mat_q8_0_wmma.hlsl. Q4_0 block = 18 bytes:
//   d(f16) + qs[16] (each byte holds 2 nibbles)
//   val(l)        = ((qs[l]      & 0x0F) - 8) * d  for l in 0..15
//   val(l + 16)   = ((qs[l] >> 4)         - 8) * d  for l in 0..15
// BK=32 == QK4_0 so each K-tile lives inside a single Q4_0 block per row.
// Note: Q4_0 block stride is 18 bytes (2-aligned, not 4-aligned), so reads
// must go through read_byte_q4_0 / read_f16_q4_0 (aligned Load + bit-shift).
//
...
uint read_byte_q4_0(ByteAddressBuffer buf, uint byte_off) {
...
void main(uint3 gid : SV_GroupID, uint3 gtid : SV_GroupThreadID) {
    ...
    for (uint kt = 0; kt < num_k_tiles; kt++) {
        ...
        // Load tile_b: BK × BN from src0 — dequantize Q4_0 (1024 elements, 256 threads, 4 each).
        // BK == QK4_0 so all 32 K-elements for a given (k_tile, row) live in the same Q4_0 block;
        // the per-row scale is read once per element here (the cache makes this near-free).
        {
            uint base = flat_id * 4;
            [unroll] for (uint e = 0; e < 4; e++) {
                uint idx = base + e;
                ...
                if (global_k < K && global_n < ne01) {
                    uint block_idx = global_k / QK4_0;
                    uint elem_in_block = global_k % QK4_0;
                    uint row_off  = src0_offset + global_n * nb01
                                  + i2_src0 * nb02 + i3_src0 * nb03;
                    uint block_off = row_off + block_idx * Q4_0_BSIZE;
                    float d  = read_f16_q4_0(src0, block_off);
                    uint byte_idx = elem_in_block % 16;
                    uint qs_byte  = read_byte_q4_0(src0, block_off + 2u + byte_idx);
