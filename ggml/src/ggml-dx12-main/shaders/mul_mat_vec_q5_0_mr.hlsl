// mul_mat_vec_q5_0_mr.hlsl - Multi-row Q5_0 matvec (M=1, 2 rows/group)
//
...
// Per-element decode given pre-loaded (uniform) qh.
...
void main(uint3 group_id : SV_GroupID, uint local_id : SV_GroupIndex) {
    ...
    for (uint block = 0; block < num_blocks; block++) {
        ...
        uint blk_off0 = src0_row0 + block * Q5_0_BSIZE;
        uint blk_off1 = src0_row1 + block * Q5_0_BSIZE;

        // Per-block scalars (d, qh) are uniform across the wave — broadcast
        // from lane 0 instead of each of 32 lanes redundantly hitting memory.
        float d0 = WaveReadLaneFirst(read_f16_v(src0, blk_off0));
        float d1 = WaveReadLaneFirst(read_f16_v(src0, blk_off1));
        uint  qh0 = WaveReadLaneFirst(read_u32_fast(src0, blk_off0 + 2));
        uint  qh1 = WaveReadLaneFirst(read_u32_fast(src0, blk_off1 + 2));

        int val0 = dequant_q5_0_qs(src0, blk_off0, qh0, elem);
        acc0 += d0 * float(val0) * x;

        int val1 = dequant_q5_0_qs(src0, blk_off1, qh1, elem);
