// mul_mat_vec_q4k_dp4a_nc.hlsli - Q4_K dp4a matvec for small-M batches.
//
// NUM_ROWS=2 x NUM_COLS outputs per workgroup: decode each Q4_K super-block
// once, reuse the decoded weights across all activation columns. Closes the
// M=2..8 gap where the wmma tiled batch path (fl=4) wastes most of its 32x32
// tile. Wrappers set NUM_COLS.
//
...
//
// Dispatch: groups_x = ceil(N/2), groups_y = 1, groups_z = ne2*ne3
// Gated to ne11 <= NUM_COLS, ne12==1, ne13==1 (broadcast handled by caller).
