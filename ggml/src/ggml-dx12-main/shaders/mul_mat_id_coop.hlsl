...
//
// A 32-thread group splits K and computes two output rows for one selected
// expert slot/token. Supports F32/F16/BF16 weights with F32 activations.
...
void main(uint3 group_id : SV_GroupID, uint tid : SV_GroupIndex) {
    ...
    for (uint k = tid; k < ne00; k += GROUP_SIZE) {
        ...
        acc0 += load_auto(src0, src0_row0 + k * nb00, src0_esize) * x;
        if (row0 + 1 < ne0) {
            acc1 += load_auto(src0, src0_row1 + k * nb00, src0_esize) * x;
        }
    }
