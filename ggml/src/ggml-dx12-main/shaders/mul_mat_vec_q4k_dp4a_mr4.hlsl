// mul_mat_vec_q4k_dp4a_mr4.hlsl - dp4a Q4_K matvec, 4 rows/group
//
// 4-row variant of mul_mat_vec_q4k_dp4a.hlsl. Q4_K has expensive per-block
// scale decoding (3 raw scale loads + several bit-twiddles per row); the
// stock 2-row shader pays that decode twice per block. This shader pays it
// 4× per block, but amortises the Q8_1 activation loads (4 packed activations
// + 4 q8_psum dp4as) across all 4 rows in registers. Halves dispatch count
// vs the 2-row variant.
//
// Reference: Vulkan ggml-vulkan.cpp:4274-4285 raises `rm_kq=4` on AMD GCN
// for the same reason.
//
...
// Worst-case LDS: 4 rows × 64 waves (GROUP_SIZE=256 / HW wave=4).
groupshared float shared_acc_mr4[NUM_ROWS][64];

// Decode Q4_K scales for one row's block. Produces sc0..sc7.
void decode_q4k_row_mr4(uint block_off, uint v_im, uint q_offset,
...
float compute_dp4a_row_mr4(float dall, float dmin,
...
void main(uint3 group_id : SV_GroupID, uint tid : SV_GroupIndex) {
    ...
    float acc[NUM_ROWS];
    [unroll] for (uint r = 0; r < NUM_ROWS; r++) acc[r] = 0.0f;

    for (uint block_idx = ix; block_idx < num_blocks; block_idx += it_size) {
        // --- Load Q8_1 data once (shared across all 4 rows) ---
        ...
        // --- Per-row decode + dp4a chain ---
        [unroll]
        for (uint r = 0; r < NUM_ROWS; r++) {
            float dall, dmin, sc0, sc1, sc2, sc3, sc4, sc5, sc6, sc7;
            uint qs0, qs64;
            decode_q4k_row_mr4(src0_rows[r] + block_idx * Q4K_BSIZE, v_im, q_offset,
                               dall, dmin, sc0, sc1, sc2, sc3, sc4, sc5, sc6, sc7, qs0, qs64);
            acc[r] += compute_dp4a_row_mr4(dall, dmin, sc0, sc1, sc2, sc3, sc4, sc5, sc6, sc7,
    ...
    // Wave-portable reduction (matches mul_mat_vec_q4k_dp4a.hlsl pattern).
    ...
    for (uint r = 0; r < NUM_ROWS; r++) {
        ...
        if (WaveIsFirstLane()) {
            shared_acc_mr4[r][wave_id] = wave_sum;
        }
    }
    ...
    if (tid == 0) {
        ...
        for (uint r = 0; r < NUM_ROWS; r++) {
            if ((row0 + r) < ne0) {
                float result = shared_acc_mr4[r][0];
                for (uint w = 1; w < num_waves; w++) result += shared_acc_mr4[r][w];
