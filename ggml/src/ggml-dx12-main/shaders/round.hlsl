// round.hlsl - ROUND unary: dst = round(src0), C99 roundf semantics
// (round half away from zero, not the HLSL round()=round-half-to-even).
...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    float r = sign(x) * floor(abs(x) + 0.5f);
