// Tiled Q4_K x Q8_1 batch MUL_MAT using packed int8 dot products.
//
...
uint read_byte_q4k_intdot(ByteAddressBuffer buf, uint byte_off) {
...
void main(uint3 gid : SV_GroupID, uint3 gtid : SV_GroupThreadID) {
    ...
    for (uint block = 0; block < num_q8_blocks; ++block) {
        ...
        if (global_n < ne01) {
            ...
            uint block_off = row_off + q4_block * Q4K_BSIZE;
            uint raw_qs = src0.Load(block_off + 16u + il * 32u + packed_idx * 4u);
            ...
            if (packed_idx == 0u) {
                ...
                uint sc = (read_byte_q4k_intdot(src0, scales_off + scidx0) & 0x0Fu)
                        | ((read_byte_q4k_intdot(src0, scales_off + scidx1) & scmask1) >> scshift1);
                uint m = ((read_byte_q4k_intdot(src0, scales_off + mbidx0) & mbmask0) >> mbshift0)
                       | ((read_byte_q4k_intdot(src0, scales_off + mbidx1) & mbmask1) >> mbshift1);
