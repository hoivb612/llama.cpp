// set_rows_q5_1.hlsl - SET_ROWS with F32 src0 -> Q5_1 dst.
//
// Vulkan parity: copy_to_quant.comp DATA_A_Q5_1 quantize.
// One thread per Q5_1 block (32 F32 elements). 32 threads per workgroup.
// Block layout: { uint16_t d; uint16_t m; uint8_t qh[4]; uint8_t qs[16]; } = 24 bytes.
...
void main(uint3 gid : SV_GroupID, uint local_id : SV_GroupThreadID) {
    ...
    uint dst_block_off = dst_offset + bi0 * 24u + (uint)row_idx * nb1 + i2 * nb2 + i3 * nb3;
    ...
    [unroll] for (uint k = 0; k < 8; ++k) {
        ...
        float x0_hi = (asfloat(src0.Load(off0 + (b0 + (QK5_1/2u)) * nb00)) - vmin) * id;
        float x1_lo = (asfloat(src0.Load(off0 + (b1)              * nb00)) - vmin) * id;
        float x1_hi = (asfloat(src0.Load(off0 + (b1 + (QK5_1/2u)) * nb00)) - vmin) * id;
        uint  xi0_lo = (uint)(x0_lo + 0.5f);
        uint  xi0_hi = (uint)(x0_hi + 0.5f);
        uint  xi1_lo = (uint)(x1_lo + 0.5f);
        uint  xi1_hi = (uint)(x1_hi + 0.5f);
        uint  byte0  = (xi0_lo & 0xFu) | ((xi0_hi & 0xFu) << 4);
        uint  byte1  = (xi1_lo & 0xFu) | ((xi1_hi & 0xFu) << 4);
        uint16_t packed = (uint16_t)(byte0 | (byte1 << 8));
        // qs starts at offset 2 (d) + 2 (m) + 4 (qh) = 8.
        dst.Store<uint16_t>(dst_block_off + 8u + k * 2u, packed);

        qh |= ((xi0_lo & 0x10u) >> 4) << (b0 + 0u);
        qh |= ((xi0_hi & 0x10u) >> 4) << (b0 + (QK5_1/2u));
        qh |= ((xi1_lo & 0x10u) >> 4) << (b1 + 0u);
        qh |= ((xi1_hi & 0x10u) >> 4) << (b1 + (QK5_1/2u));
    }
    dst.Store<uint16_t>(dst_block_off + 4u,      (uint16_t)(qh & 0xFFFFu));
    dst.Store<uint16_t>(dst_block_off + 4u + 2u, (uint16_t)((qh >> 16) & 0xFFFFu));
}
