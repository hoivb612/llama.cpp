// get_rows_q8_1.hlsl - Gather and dequantize rows from Q8_1 source
...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    uint block_idx = i0 / QK8_1;
    uint elem_in_block = i0 % QK8_1;

    uint row_off = src0_offset + (uint)row_idx * nb01 + i2 * nb02 + i3 * nb03;
    uint block_off = row_off + block_idx * Q8_1_BSIZE;

    float d = read_f16_gr(src0, block_off);
    int qs = read_sbyte_gr(src0, block_off + 4 + elem_in_block);
