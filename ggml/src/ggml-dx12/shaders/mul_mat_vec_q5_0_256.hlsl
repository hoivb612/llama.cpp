// mul_mat_vec_q5_0_256.hlsl - Q5_0 matvec with 256 threads (for large K)
// Same algorithm as mul_mat_vec_q5_0.hlsl but with 256 threads + shared memory reduction
// Better for large K (>=1024), worse for small K (<512)
...
uint read_u32_v(ByteAddressBuffer buf, uint byte_off) {
    return read_byte_v(buf, byte_off) | (read_byte_v(buf, byte_off+1) << 8) |
           (read_byte_v(buf, byte_off+2) << 16) | (read_byte_v(buf, byte_off+3) << 24);
}
...
void main(uint3 group_id : SV_GroupID, uint tid : SV_GroupIndex) {
    ...
    for (uint k = tid; k < K; k += GROUP_SIZE) {
        ...
        uint qh = read_u32_v(src0, block_off + 2);

        int val;
        if (elem < 16) {
            uint qs_byte = read_byte_v(src0, block_off + 6 + elem);
            uint xh = ((qh >> elem) << 4) & 0x10u;
            val = (int)((qs_byte & 0x0Fu) | xh) - 16;
        } else {
            uint jj = elem - 16;
            uint qs_byte = read_byte_v(src0, block_off + 6 + jj);
