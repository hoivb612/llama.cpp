// mul_mat_vec_q4k_mr.hlsl - Multi-row matrix-vector multiply for Q4_K (M=1)
//
// Processes 2 output rows per thread group, sharing activation loads
// between rows. This halves activation memory traffic since the same
// input vector is reused for both rows.
//
...
void main(uint3 group_id : SV_GroupID, uint tid : SV_GroupIndex) {
    ...
    // Thread-to-block mapping (same as single-row Q4_K)
    ...
    for (uint block_idx = ix; block_idx < num_blocks; block_idx += it_size) {
        ...
        // === Row 0: dequantize and dot product ===
        {
            uint block_off = src0_row0 + block_idx * Q4K_BSIZE;
        ...
        // === Row 1: dequantize and dot product (reuse activations) ===
        if (valid_row1) {
            uint block_off = src0_row1 + block_idx * Q4K_BSIZE;
    ...
    // === Parallel reduction for both rows (2 barriers, same as single-row) ===
    ...
    if (WaveIsFirstLane()) {
        ...
        shared_acc[wave_id + num_waves] = wave_sum1;
    }
    GroupMemoryBarrierWithGroupSync();

    if (tid < num_waves) {
        float v0 = shared_acc[tid];
        float v1 = shared_acc[tid + num_waves];
        v0 = WaveActiveSum(v0);
        v1 = WaveActiveSum(v1);
        if (tid == 0) {
            shared_acc[0] = v0;
    ...
    // === Write results for both rows ===
    if (tid == 0) {
        float r0 = shared_acc[0];
        if (op0 == 1u) r0 += asfloat(src2.Load(op1 + row_base * 4));
        uint off0 = offset_4d(row_base, 0, i2, i3, nb0, nb1, nb2, nb3, dst_offset);
        store_auto(dst, off0, r0, dst_esize);

        if (valid_row1) {
            float r1 = shared_acc[1];
            if (op0 == 1u) r1 += asfloat(src2.Load(op1 + (row_base + 1) * 4));
            uint off1 = offset_4d(row_base + 1, 0, i2, i3, nb0, nb1, nb2, nb3, dst_offset);
            store_auto(dst, off1, r1, dst_esize);
        }
    }
}
