// mul_mat_vec_q4k_32.hlsl - Wave-only Q4_K matvec (32 threads, no barriers)
//
// Same algorithm as mul_mat_vec_q4k.hlsl but uses 32 threads:
// - 16 threads cooperate per Q4_K block (same as 256t variant)
// - 2 blocks processed per iteration (it_size = 32/16 = 2)
// - WaveActiveSum replaces shared memory reduction (zero barriers)
// - Optimal for UMA: 51.2 occupancy/CU, no stalls
//
...
void main(uint3 group_id : SV_GroupID, uint tid : SV_GroupIndex) {
    uint i0 = group_id.y * 65535u + group_id.x;
    ...
    // 16 threads cooperate per Q4_K block; 32 threads = 2 blocks per iteration
    uint it_size = GROUP_SIZE / 16;  // 2
    uint itid = tid % 16;
    uint ix = tid / 16;
    ...
    // Wave-only reduction — no barriers, no shared memory
