...
//
// WARNING: Start foreign code.
//
//          The following include code is not part of the the zo sources. It is added
//          to enable setting process/thread affinity, disabling core parking, and to
//          enumerate the cache configuration.
//

#include "config.inc"

//
// WARNING: End foreign code.
//
...
void ggml_vec_dot_bf16(int n, float * restrict s, size_t bs, const ggml_bf16_t * restrict x, size_t bx, const ggml_bf16_t * restrict y, size_t by, int nrc);
...
//
// ******* WARNING: This is simple version of this function used only to enable
...
#define GGML_F32_EPR8 8
    ...
    const uint64_t xn = (nc & ~(GGML_F32_EPR8 - 1));
    ...
    if (xn) {
        ...
        const uint64_t np = (nc & ~(GGML_F32_STEP8 - 1));
        ...
        if (np) {
            do {
                for (uint64_t j = 0; j < GGML_F32_ARR; j++) {
                    au[j] = _mm_loadu_si128((__m128i *)(x + i + j * GGML_F32_EPR8));
...
//
// ******* WARNING BEGIN: These functions, unfortunatly, do not exist in the zo-ggml code.
...
// This is a new API being exported from ggml.
...
void ggml_vec_sumsq_bf16(const uint64_t n, float * s, const ggml_bf16_t * x) {
    ...
    ggml_vec_dot_bf16(n, s, 0, x, 0, x, 0, 1);
}
...
void ggml_barrier(struct ggml_compute_state * state) {
    ...
    int pn_threads = atomic_fetch_add(n_barrier, 1);
...
{
    puts("Tensor waits emcompass init, compute, and finalize waits\n\n");
