...
void llbench_log_callback(ggml_log_level level, const char * text, void * user_data) {
    GGML_UNUSED(text);

    ggml_log_level llbench_log_level = (ggml_log_level)0 /* GGML_LOG_LEVEL_NONE */;
    if (user_data != nullptr) {
        llbench_log_level = *(ggml_log_level *)user_data;
    }

    if (level == llbench_log_level) {
        fputs(text, stdout);
    }
}
...
int main(int argc, char ** argv) {
    ...
    llama_log_set(llbench_log_callback, &(params.log_id));
    ...
    params.log_id = GGML_LOG_LEVEL_INFO;
    llama_log_set(llbench_log_callback, &(params.log_id));
