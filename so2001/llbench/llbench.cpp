...
static const char * output_format_str(output_formats format) {
    switch (format) {
        ...
        case SQL:      return "sql";
        default: GGML_ASSERT("invalid output format"); return "";
    }
}
...
static const char * split_mode_str(llama_split_mode mode) {
    switch (mode) {
        ...
        default: GGML_ASSERT("invalid split mode"); return "";
    }
}
...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            use_omp = true;
        } else if (arg == "-repack" || arg == "--repack_mode") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params.repacking_mode = std::stoi(argv[i]);
        } else if (arg == "repack-xbox") {
            params.repacking_mode = 2;
        } else if (arg == "repack-ggml") {
            params.repacking_mode = 1;
        } else if (arg == "repack-xbcg") {
            params.repacking_mode = 3;
        } else {
            invalid_param = true;
            break;
        }
...
const std::string test::build_commit = "6a9f5df6"; // LLAMA_COMMIT
const int         test::build_number = 3239; // LLAMA_BUILD_NUMBER
...
#ifdef __TENSOR_REPACK__
    ggml_tensor_repacking_mode_t repacking_mode = TENSOR_REPACKING_MODE_NONE;
    switch (params.repacking_mode) {
        case 0: repacking_mode = TENSOR_REPACKING_MODE_NONE; break;
        case 1: repacking_mode = TENSOR_REPACKING_MODE_GGML; break;
        case 2: repacking_mode = TENSOR_REPACKING_MODE_XBOX; break;
        case 3: repacking_mode = TENSOR_REPACKING_MODE_XBCG; break;
        default: break;
    }
    ggml_set_tensor_repacking_mode(repacking_mode);
#endif // __TENSOR_REPACK__
