...
#pragma warning (disable:4242) // conversion from 'int' to 'size_t'
#pragma warning (disable:4297) // func assumed not to throw but does
...
#else
        // 
        // to enable tensor data repacking without modifying original mapped file
        //
...
#if defined(__TENSOR_REPACK__)

    // 
    // attempt to repack tensor as the callgraph is built
    //

    ggml_repack_tensor_callgraph(result);
    ...
    llm_tokenizer_bpe(const llama_vocab & vocab): vocab(vocab) {
        ...
        switch (vocab.type_pre) {
            ...
            case LLAMA_VOCAB_PRE_TYPE_DEEPSEEK_LLM:
                regex_exprs = {
                    "[\r\n]",
                    "\\s?[A-Za-zµÀ-ÖØ-öø-ƺƼ-ƿǄ-ʓʕ-ʯͰ-ͳͶͷͻ-ͽͿΆΈ-ΊΌΎ-ΡΣ-ϵϷ-ҁҊ-ԯԱ-ՖႠ-ჅᎠ-Ᏽᏸ-ᏽᲐ-ᲺᲽ-Ჿᴀ-ᴫᵫ-ᵷᵹ"
                    "-ᶚḀ-ἕἘ-Ἕἠ-ὅὈ-Ὅὐ-ὗὙὛὝὟ-ώᾀ-ᾴᾶ-ᾼιῂ-ῄῆ-ῌῐ-ΐῖ-Ίῠ-Ῥῲ-ῴῶ-ῼℂℇℊ-ℓℕℙ-ℝℤΩℨK-ℭℯ-ℴℹℼ-ℿⅅ-ⅉⅎↃↄⰀ-ⱻⱾ-"
                    "ⳤⳫ-ⳮⳲⳳꙀ-ꙭꚀ-ꚛꜢ-ꝯꝱ-ꞇꞋ-ꞎꭰ-ꮿﬀ-ﬆﬓ-ﬗＡ-Ｚａ-ｚ𐐀-𐑏𐒰-𐓓𐓘-𐓻𐲀-𐲲𐳀-𐳲𑢠-𑣟𞤀-𞥃]+",
                    "\\s?[!-/:-~！-／：-～‘-‟　-。]+",
                    "\\s+$",
                    "[一-龥ࠀ-一가-퟿]+",
                    "\\p{N}+",
...
void llama_print_timings(struct llama_context * ctx) {
    ...
    printf("\n");
    printf("       load time = %10.2f ms\n", timings.t_load_ms);
    printf("     sample time = %10.2f ms / %5d runs   (%8.2f ms per token, %8.2f tokens per second)\n",
            timings.t_sample_ms, timings.n_sample, timings.t_sample_ms / timings.n_sample, 
            (timings.t_sample_ms == 0) ? 0.0 : 1e3 / timings.t_sample_ms * timings.n_sample);

    printf("prompt eval time = %10.2f ms / %5d tokens (%8.2f ms per token, %8.2f tokens per second)\n",
            timings.t_p_eval_ms, timings.n_p_eval, timings.t_p_eval_ms / timings.n_p_eval, 
            (timings.t_p_eval_ms == 0) ? 0.0 : 1e3 / timings.t_p_eval_ms * timings.n_p_eval);

    printf("       eval time = %10.2f ms / %5d runs   (%8.2f ms per token, %8.2f tokens per second)\n",
            timings.t_eval_ms, timings.n_eval, timings.t_eval_ms / timings.n_eval, 
            (timings.t_eval_ms == 0) ? 0.0 : 1e3 / timings.t_eval_ms * timings.n_eval);

    printf("      total time = %10.2f ms / %5d tokens\n", (timings.t_end_ms - timings.t_start_ms), (timings.n_p_eval + timings.n_eval));
