#pragma warning (disable:4756) // overflow in constant arithmetic (-INFINITY)
...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "-t" || arg == "--threads") {
        ...
        int32_t n_threads = std::stoi(argv[i]);
        if (n_threads <= 0) {
...
#else
    ggml_vec_mul1_f32(n, out, inp, norm);
#endif // #if 0
}
...
#else
    return ggml_cosine_similarity_f32(n, embd1, embd2);
#endif // #if 0
}
