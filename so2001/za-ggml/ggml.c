#pragma warning (disable:4756) // overflow in const arithmetic
...
void ggml_vec_abs_f32(const uint64_t n, float * y, const float * x)
{
    ...
    for (; i < np; i += GGML_F32_STEP16) {
        for (uint64_t j = 0; j < GGML_F32_ARR; j++) {
            ax[j] = _mm512_abs_ps(_mm512_loadu_ps(x + i + j * GGML_F32_EPR16));
            _mm512_storeu_ps((y + i + j * GGML_F16_EPR16), ax[j]);
        }
    }
    ...
    for (; i < xn; i += GGML_F32_EPR16) {
        ax[0] = _mm512_abs_ps(_mm512_loadu_ps(x + i));
        _mm512_storeu_ps((y + i), ax[0]);
    }
    ...
    )
{
    ...
    if (mul_mat_repack_count) {
        printf("total number of repack callgraphs %d\n", mul_mat_repack_callgraph_count);
        printf("total number of quant blocks processed %d\n", mul_mat_repack_quants);
        ...
        printf("average conversion time per quant block %5.2fus\n",
               (float)mul_mat_repack_time_us / (float)mul_mat_repack_quants); 

        printf("shared repack conversions %d\n\n", mul_mat_repack_shared);
...
struct ggml_numa_nodes {
    ...
    uint32_t cpuset; // no NUMA support - use a portable datatype
};
...
void ggml_compute_forward_dup_same_cont(
    ...
    if (ie0 < ie1) {
        ...
        //
        // N.B. Copy f32 is a beneign move and copies data with no conversions.
        //
...
void ggml_compute_forward_mul_mat(
    ...
    if (src1_type == GGML_TYPE_F32 &&
        ...
        if (!ith && (src0_type != src0->type)) {
            ...
            mul_mat_repack_quants += (src0->ne[0] * src0->ne[1]) / QK_K;
        ...
        if (!ith &&
            ((src0_type == GGML_TYPE_Q4_0_8_8) ||
             (src0_type == GGML_TYPE_Q4_K_8_8) ||
             (src0_type == GGML_TYPE_Q4_0_x8) ||
             (src0_type == GGML_TYPE_Q8_0_x8) ||
             (src0_type == GGML_TYPE_Q2_K_x8) ||
             (src0_type == GGML_TYPE_Q3_K_x8) ||
             (src0_type == GGML_TYPE_Q4_K_x8) ||
             (src0_type == GGML_TYPE_Q6_K_x8))) {
...
//        printf("unused cpu %d in mul mat\n", ith);
    ...
    //
    // N.B. The computed block factor is zero if the available space in the l1d_cache
    //      is less than the src0 row size.
    //
...
thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    if (ith && ggml_set_thread_priority(ith)) {
    ...
    for (uint32_t node_n = 0; node_n < graph_n_nodes; node_n++) {
        ...
        if (ith < n_tasks) {
            ...
            if (!ith) {
                ...
                //
                // Update unary op counts and op time.
                //

                if (op == GGML_OP_UNARY) {
                    enum ggml_unary_op u_op = ggml_get_unary_op(node);
                    unary_op_counts[u_op] += 1;
                    unary_op_time[u_op] += tensor_t0;
                }
            }
...
void ggml_repack_tensor_callgraph(struct ggml_cgraph *cgraph) {
    ...
    if (tensor_repacking_mode != TENSOR_REPACKING_MODE_XBCG) {
        return;
    }
    ...
    //
    // Create a NULL params block for a single-thread tensor repacking
    //
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes; node_n++) {
        ...
        if (tensor->is_skipped) {
            continue;
        }

        //
        // repack only applies to MUL_MAT
        //
...
static int gguf_get_or_add_key(struct gguf_context * ctx, const char * key) {
    ...
    ctx->kv[n_kv].key.data = _strdup(key);
    ctx->header.n_kv++;
...
void gguf_set_val_str(struct gguf_context * ctx, const char * key, const char * val) {
    ...
    ctx->kv[idx].value.str.data = _strdup(val);
}
...
void gguf_add_tensor(
    ...
    ctx->infos[idx].name.data = _strdup(tensor->name);
