...
// =====================================================================================
// Gemma 4 MTP single-step graph builder
// =====================================================================================
...
static void gemma4_mtp_build_one_step(
        ...
        ggml_tensor * tok_step,
        ggml_tensor * h_step,
        ggml_tensor * pos_step,
        ggml_tensor ** out_logits,
        ggml_tensor ** out_h_post,
        ggml_tensor ** out_argmax) {
    auto *       ctx0    = gctx.ctx0;
    auto *       gf      = gctx.gf;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        const int   n_rot_l      = (int) hparams.n_rot(il);
        ...
        Qcur = ggml_rope_ext(ctx0, Qcur, pos_step, freq_factors, n_rot_l, rope_type, n_ctx_orig,
        ...
        GGML_ASSERT(il_kv >= 0 && "Gemma4 MTP: target has no layer matching MTP attention type");

        // Even when target's attention_k_eq_v=True, the V cache is still written with
        // the un-rotated tensor, so fetch V from the cache (never reuse K).
    ...
    if (mtp.hparams.use_ordered_embeddings) {
        ...
        ggml_tensor * ordering = ggml_view_2d(ctx0, mtp.mtp_token_ordering, vsc, (int64_t) n_c, ordering_nb1, 0);
