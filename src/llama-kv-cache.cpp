...
llama_kv_cache_context::llama_kv_cache_context(
    ...
    // Pre-compute n_kv so read-only paths (e.g. MTP draft, which does NOT call apply())
    // get a valid mask/K/V shape based on current cache occupancy. For paths that DO
    // call apply(), n_kv is re-derived there after apply_ubatch().
